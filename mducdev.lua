...
NotificacaoNevoHub("NEVO HUB 🔔", "Script Version : V4.5")
...
spawn(function()
  while task.wait() do
    ...
    Test.OutlineColor = Color3.fromRGB(0, 170, 255)
...
local Window = redzlib:MakeWindow({
  Title = "<font color='rgb(0,170,255)'>NEVO HUB </font>",
  SubTitle = "<font color='rgb(0,170,255)'>- Blox Fruit</font>",
...
-- NEVO HUB: đổi accent đỏ của UI hiện tại sang xanh, không thay UI.
task.delay(1, function()
    pcall(function()
        local BLUE = Color3.fromRGB(0, 170, 255)
        local function isRed(c)
            return c.R > 0.55 and c.R > c.G * 1.5 and c.R > c.B * 1.5
        end
        local function scan(root)
            if not root then return end
            for _, o in ipairs(root:GetDescendants()) do
                pcall(function()
                    if o:IsA("UIStroke") and isRed(o.Color) then
                        o.Color = BLUE
                    elseif (o:IsA("Frame") or o:IsA("ScrollingFrame")
                        or o:IsA("TextButton") or o:IsA("ImageButton"))
                        and isRed(o.BackgroundColor3) then
                        o.BackgroundColor3 = BLUE
                    elseif (o:IsA("TextLabel") or o:IsA("TextButton"))
                        and isRed(o.TextColor3) then
                        o.TextColor3 = BLUE
                    end
                end)
            end
        ...
        if p then scan(p:FindFirstChildOfClass("PlayerGui")) end
    end)
end)
