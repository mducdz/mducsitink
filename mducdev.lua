...
-- ============================================================
-- Fluent Maru UI
-- Compatibility layer: keeps the feature code from message.txt
-- while rendering it through Fluent.
-- ============================================================
local Fluent
local ok, err = pcall(function()
    Fluent = loadstring(game:HttpGet(
        "https://raw.githubusercontent.com/xshiba/OBFUP/refs/heads/main/GET.lua"
    ))()
end)

if not ok or not Fluent then
    warn("[NEVO] Không load được Fluent UI:", err)
    return
end
...
-- Fluent uses MinimizeKey; the old custom minimize button is therefore
-- intentionally not duplicated.

local function NormalizeControlOptions(options)
    ...
    -- message.txt uses Redz-style "Name"; Fluent Maru uses "Title".
...
local function WrapParagraph(paragraph)
    ...
    -- Preserve the SetDesc calls already used throughout message.txt.
    if type(paragraph.SetDesc) ~= "function" and type(paragraph.SetDescription) == "function" then
        paragraph.SetDesc = paragraph.SetDescription
    end
...
local function MakeFluentTab(definition)
    ...
    function adapter:AddParagraph(title, description)
        if type(title) == "table" then
            return WrapParagraph(tab:AddParagraph(NormalizeControlOptions(title)))
        end

        return WrapParagraph(tab:AddParagraph({
            Title = tostring(title or ""),
            Content = tostring(description or "")
        }))
    end

    -- Fallback for any direct Fluent tab method not explicitly wrapped.
...
-- ============================================================
-- Original feature implementation from message.txt starts here.
-- Its original Redz UI bootstrap is removed/replaced above.
-- ============================================================
...
-- Fluent Maru load complete
task.defer(function()
    AddFluentNotify("NEVO Hub", "Fluent Maru UI đã load thành công.", 5)
end)
